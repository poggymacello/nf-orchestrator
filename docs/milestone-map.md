...
| **M4** | Observability + 3 failure drills + postmortems + runbooks | learning-notes/prometheus-instrumentation, operations/runbooks, operations/postmortems, ADR-0006 | code: `/metrics`, reconciler fixes and `/readyz` done · drills 1–2 done and their findings fixed, drill 3 blocked (kind disables disk eviction) · docs: done except drill 3 |
| **M5** | CI hardening — Gitleaks, Trivy, e2e with kind | docs on CI security gates | planned |
| **M6** | LLM intent layer (text → validated JSON) with guardrails | ADR-0007 (LLM guardrail) | planned |
